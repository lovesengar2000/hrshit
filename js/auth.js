...
class AuthAPI {
        ...
        try {
            ...
            async getAllUsersLeaveSummaries(companyId, authToken) {
                try {
                    ...
                    if (typeof token === 'string' && token.startsWith('Bearer ')) tokenToParse = token.slice(7);
                    ...
                    const types = (typesRes && typesRes.success && Array.isArray(typesRes.data)) ? typesRes.data : [];
                    const typeMap = {};
                    types.forEach(t => {
                    ...
                    leaves.forEach(leave => {
                        const uid = (leave.userId || leave.employeeId || (leave.employee && (leave.employee.userId || leave.employee.id)) || leave.appliedBy || 'unknown').toString();
                        if (!usersMap[uid]) {
                            usersMap[uid] = {
                                userId: uid,
                                userName: leave.employeeName || leave.userName || leave.applicantName || null,
                                leaves: []
                            };
                        }
                    ...
                    const getTypeKey = (leave) => {
                        ...
                        const name = (leave.leaveTypeName || (leave.leaveType && (leave.leaveType.name || leave.leaveTypeId)) || leave.leaveTypeId || leave.code || '').toString();
                    ...
                    Object.keys(usersMap).forEach(uid => {
                        ...
                        entry.leaves.forEach(leave => {
                            ...
                            if (leave.status === 'APPROVED') {
                                const start = leave.startDate ? new Date(leave.startDate) : (leave.createdAt ? new Date(leave.createdAt) : null);
                                const end = leave.endDate ? new Date(leave.endDate) : start;
                                if (start && end) {
                                    const days = Math.ceil((end - start) / (1000 * 60 * 60 * 24)) + 1;
                                    summaryMap[key].usedDays += days;
                            const url = `${this.BASE_URL}/leave/balance?companyId=${encodeURIComponent(company)}&employeeId=${encodeURIComponent(eid)}`;
                                }
                            }
                        });

                        // Compute CL/EL aggregates
                        const aggregate = { CL: { total: 0, used: 0, remaining: 0 }, EL: { total: 0, used: 0, remaining: 0 } };
                        Object.keys(summaryMap).forEach(k => {
                            const item = summaryMap[k];
                            const name = (item.typeName || k).toLowerCase();
                            if (name.includes('casual') || name === 'cl' || name.includes('c l') || name.includes('cas')) {
                                aggregate.CL.total += item.totalDays || 0;
                                aggregate.CL.used += item.usedDays || 0;
                                aggregate.CL.remaining = Math.max(0, aggregate.CL.total - aggregate.CL.used);
                            } else if (name.includes('earn') || name.includes('annual') || name === 'el' || name.includes('el ')) {
                                aggregate.EL.total += item.totalDays || 0;
                                aggregate.EL.used += item.usedDays || 0;
                                aggregate.EL.remaining = Math.max(0, aggregate.EL.total - aggregate.EL.used);
                            }
                        });
            ...
            async getLeaveBalanceFromAllRequests(companyId, employeeId, authToken) {
                try {
                    ...
                    const tokenData = this.parseToken(tokenToParse);
                    if (!tokenData || tokenData.role !== 'COMPANY_ADMIN') {
                        return { success: false, status: 403, data: null, message: 'Access denied. Admins only.' };
                    }

                    if (!employeeId) {
                        return { success: false, data: null, message: 'employeeId is required' };
                    }

                    // fetch all leaves for company
                    const allRes = await this.getAllLeaveRequestsAdmin(company, tokenToParse);
                    if (!allRes || !allRes.success || !Array.isArray(allRes.data)) {
                        return { success: false, data: null, message: 'Failed to fetch leave requests' };
                    }
                    const leaves = allRes.data;
                    ...
                    const userLeaves = leaves.filter(l => {
                        return (
                            ...
                            (l.employee && (String(l.employee.id) === String(employeeId) || String(l.employee.employeeId) === String(employeeId))) ||
                    ...
                    userLeaves.forEach(leave => {
                        ...
                        if (leave.status === 'APPROVED') {
                            ...
                            if (start && end) {
                                const days = Math.ceil((end - start) / (1000 * 60 * 60 * 24)) + 1;
                                summaryMap[key].usedDays += days;
                                summaryMap[key].remainingDays = Math.max(0, summaryMap[key].totalDays - summaryMap[key].usedDays);
                            }
                        }
                    });
    ...
    async getCurrentUserEmployeeId() {
        try {
            ...
            if (user) {
                ...
                if (user.employee && (user.employee.employeeId || user.employee.id)) return user.employee.employeeId || user.employee.id;
                if (user.profileEmployeeId) return user.profileEmployeeId;
                if (user.profile && (user.profile.employeeId || user.profile.id)) return user.profile.employeeId || user.profile.id;
                if (user.data && (user.data.employeeId || user.data.id)) return user.data.employeeId || user.data.id;
    ...
    async getUserLeaveRequests() {
        try {
            ...
            const userLeaves = allLeaves.data.filter(leave => 
                leave.userId === userId || 
                leave.employeeId === userId ||
                (leave.employee && (leave.employee.userId === userId || leave.employee.id === userId))
            );
    ...
    async getEmployeeLeaveRequests(employeeId) {
        try {
            ...
            const employeeLeaves = allLeaves.data.filter(leave => 
                leave.userId === employeeId || 
                leave.employeeId === employeeId ||
                (leave.employee && (leave.employee.userId === employeeId || leave.employee.id === employeeId))
            );
    ...
    async getUserLeaveBalance() {
        try {
            ...
            userLeaves.data.forEach(leave => {
                ...
                if (leaveDate.getFullYear() === currentYear && leave.status === 'APPROVED') {
                    ...
                    if (leaveTypeId && balance[leaveTypeId]) {
                        ...
                        balance[leaveTypeId].remainingDays = Math.max(0, balance[leaveTypeId].maxDays - balance[leaveTypeId].usedDays);
                    }
                }
            });
    ...
    async getAllLeaveRequests() {
        try {
            ...
            const url = `${this.BASE_URL}/leave/allLeaveRequest?companyId=${encodeURIComponent(company)}`;
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'authorization': `Bearer ${token}`,
    ...
    async getUserLeaveBalance() {
        try {
            ...
            const currentYear = new Date().getFullYear();
            const approvedLeaves = userLeaves.data.filter(leave => 
        ...
        try {
            ...
            if (!tokenData || tokenData.role !== 'COMPANY_ADMIN') {
                return {
                    success: false,
                    status: 403,
                    data: { message: 'Access denied. Only admins can create leave types.' }
                };
            }
    ...
    async applyLeave(companyIdOrData, leaveTypeId, startDate, endDate, reason, authToken) {
        try {
            ...
            const tokenToParse = typeof token === 'string' && token.startsWith('Bearer ') ? token.slice(7) : token;
            const tokenData = this.parseToken(tokenToParse);
            if (!tokenData) {
                return {
                    success: false,
            ...
            if (!payload.employeeId) {
                payload.employeeId = tokenData.employeeId || tokenData.employeeID || tokenData.employee || null;
                if (!payload.employeeId) {
    ...
    async getLeaveBalance(companyId, employeeId, authToken) {
        try {
            ...
            let eid = employeeId || (tokenData && (tokenData.employeeId || tokenData.employeeID || tokenData.employee));
            ...
            if (tokenData && tokenData.role === 'COMPANY_ADMIN') {
                return await this.getLeaveBalanceFromAllRequests(company, eid, tokenToParse);
            }
                ...
                try {
                    ...
                    if (!tokenData || tokenData.role !== 'COMPANY_ADMIN') {
                        return {
                            success: false,
                            status: 403,
                            data: { message: 'Access denied. Only admins can list all leave requests.' }
                        };
                    }
                ...
                try {
                    ...
                    if (!tokenData || tokenData.role !== 'COMPANY_ADMIN') {
                        return {
                            success: false,
                            status: 403,
                            data: { message: 'Access denied. Only admins can approve leave requests.' }
                        };
                    }
            ...
            async getLeaveRequests(companyId, authToken) {
                try {
                    ...
                    if (!currentUserId) return { success: false, data: [], message: 'User not authenticated' };
                    ...
                    const filtered = allRes.data.filter(leave => {
                        return (
                            ...
                            (leave.employee && (String(leave.employee.userId) === String(currentUserId) || String(leave.employee.id) === String(currentUserId))) ||
                            (leave.requestedBy && String(leave.requestedBy) === String(currentUserId)) ||
                ...
                async getUserLeaveSummary(companyId, userId, authToken) {
                    try {
                        ...
                        if (!targetUserId) {
                            return { success: false, data: null, message: 'No user id provided or logged in user not found' };
                        }
                        ...
                        if (tokenData && tokenData.role === 'COMPANY_ADMIN') {
                            const allRes = await this.getAllLeaveRequestsAdmin(company, tokenToParse);
                            if (allRes && allRes.success && Array.isArray(allRes.data)) leaves = allRes.data;
                        } else {
                            const userRes = await this.getUserLeaveRequests();
                            if (userRes && userRes.success && Array.isArray(userRes.data)) leaves = userRes.data;
                        }
                        ...
                        const userLeaves = leaves.filter(leave => {
                            return (
                                ...
                                (leave.employee && (String(leave.employee.userId) === String(targetUserId) || String(leave.employee.id) === String(targetUserId))) ||
