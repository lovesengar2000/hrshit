...
class Attendance {
  ...
  async loadEmployeeId() {
      ...
      try {
        ...
        if (result.success && result.data && Array.isArray(result.data)) {
          ...
          const employee = employees.find(
            (emp) =>
              emp.email &&
              emp.email.toLowerCase() === this.user.email.toLowerCase()
          );
    ...
    if (!this.employeeId) {
      this.showStatusError(
        "Cannot load attendance: Employee profile not found. Please contact admin."
      );
  ...
  async loadTodayStatus() {
    if (!this.employeeId || !this.companyId) {
      ...
      this.showStatusError(
        "Cannot load attendance: Missing profile information"
      );
      return;
    }
    ...
    try {
      ...
      this.showStatusError(
        "Error loading attendance status. Please try refreshing."
      );
      this.disableAllActions();
    }
  }
  ...
  updateStatusDisplay(status) {
    ...
    const clockInTime = status.clockInTime
      ? new Date(status.clockInTime)
      : null;
    const clockOutTime = status.clockOutTime
      ? new Date(status.clockOutTime)
      : null;
    ...
    if (clockInTime && clockOutTime) {
      ...
      hoursWorked = ((clockOutTime - clockInTime) / (1000 * 60 * 60)).toFixed(
        2
      );
    }
  ...
  updateActionButtons(status) {
    ...
    // const clockInTime = status.events[status.events.length - 1].status == "CLOCK_IN" ? status.clockInTime : null;
    // const clockOutTime =  status.events[status.events.length - 1].status == "CLOCK_OUT" ? status.clockOutTime : null;
  ...
  async handleClockIn() {
    if (!this.companyId || !this.employeeId) {
      this.showMessage(
        "Cannot clock in: Missing company or employee information",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showMessage(
          `Failed to clock in: ${result.data?.message || "Unknown error"}`,
          "error"
        );
  ...
  async handleClockOut() {
    if (!this.companyId || !this.employeeId) {
      this.showMessage(
        "Cannot clock out: Missing company or employee information",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showMessage(
          `Failed to clock out: ${result.data?.message || "Unknown error"}`,
          "error"
        );
  ...
  async loadAttendanceHistory() {
    if (!this.employeeId || !this.companyId) {
      ...
      this.showMessage(
        "Cannot load attendance history: No employee profile",
        "error"
      );
      return;
    }
  ...
  displayAttendanceHistory(dailySummaries) {
    ...
    tableBody.innerHTML = dailySummaries
      .map((summary) => {
        const clockInTime = summary.clockInTime
          ? new Date(summary.clockInTime)
          : null;
        const clockOutTime = summary.clockOutTime
          ? new Date(summary.clockOutTime)
          : null;
